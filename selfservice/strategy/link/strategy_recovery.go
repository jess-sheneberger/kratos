...
func (s *Strategy) recoveryIssueSession(w http.ResponseWriter, r *http.Request, f *recovery.Flow, id *identity.Identity) error {
	...
	if len(id.VerifiableAddresses) == 1 &&
		len(id.RecoveryAddresses) == 1 && 
		strings.ToLower(id.VerifiableAddresses[0].Value) == strings.ToLower(id.RecoveryAddresses[0].Value) {
		...
		err := s.d.PrivilegedIdentityPool().UpdateVerifiableAddress(r.Context(), &address)
	   	if err != nil {
