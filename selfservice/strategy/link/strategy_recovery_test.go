...
func TestRecovery(t *testing.T) {
	...
	t.Run("description=should not be able to recover an inactive account", func(t *testing.T) {
		var check = func(t *testing.T, recoverySubmissionResponse, recoveryEmail string, isAPI bool) {
			...
			// we changed the recovery link in the email to point at the UI, not Kratos, so make the request of 
			// kratos instead of the UI
	...
	t.Run("description=should not be able to use an outdated flow", func(t *testing.T) {
		...
		_, _, err = testhelpers.NewSDKCustomClient(public, c).V0alpha2Api.GetSelfServiceRecoveryFlow(context.Background()).Id(res.Request.URL.Query().Get("flow")).Execute()
